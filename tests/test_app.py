...
def test_auth_page(client, monkeypatch):
    # GIVEN a Flask application configured for testing
    auth_data_mock = {
        "athlete": {"firstname": "Test", "lastname": "User", "id": 1},
...
def test_contacts_page(client):
    ...
    assert b"https://www.strava.com/athletes/21151421" in response.data
    assert b"https://www.instagram.com/victor_korzunin/" in response.data
    assert b"https://github.com/floydspace" in response.data
...
def test_process_webhook_get_subscription_failed(monkeypatch, app):
    ...
    monkeypatch.setattr(strava_helpers, "is_app_subscribed", lambda: False)
    monkeypatch.setenv("STRAVA_WEBHOOK_TOKEN", "token_for_test")
    params = {"hub.mode": "subscribe", "hub.verify_token": "failed_token", "hub.challenge": "challenge_for_test"}
    values_url = urllib.parse.urlencode(params)
