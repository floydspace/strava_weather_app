...
def test_strava_client_get_activity(database, db_token, monkeypatch):
    ...
    responses.add(
        responses.GET, f"https://www.strava.com/api/v3/activities/{activity_id}", json={"athlete_id": athlete_tokens.id}
    )
...
def test_strava_client_update_tokens_failed(database, db_token, monkeypatch):
    activity_id = 1
    athlete_id = 2
    responses.add(responses.POST, "https://www.strava.com/oauth/token", body=json.dumps({"response": "bad response"}))
    monkeypatch.setattr(manage_db, "get_db", lambda: database)
