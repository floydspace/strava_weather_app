...
from utils.dill_process import DillProcess
...
app.config.from_mapping(
    ...
    DATABASE=os.path.join(app.root_path, os.environ["DATABASE"]),
)
...
def auth():
    ...
    try:
        ...
        tokens = manage_db.Tokens(
            auth_data["athlete"]["id"], auth_data["access_token"], auth_data["refresh_token"], auth_data["expires_at"]
        )
    except KeyError:
        return abort(500)
...
def process_webhook_post():
    ...
    if args["aspect_type"] == "create" and args["object_type"] == "activity":
        p = DillProcess(target=weather.add_weather, args=(args["owner_id"], args["object_id"]))
        p.daemon = True
        p.start()
